...
→ 衝突は同 part 内で検出し index でディスアンビギュエート。ID は **`ocz1:` プレフィックス**で名前空間化（`office-casual zone 1`）。
...
```bash
office-casual embed report.pptx --mode part          # → report.ocz.pptx (安全埋め込み)
office-casual embed report.docx --mode both          # part + 属性注入
```
```ts
import { embedFile, readDataPart } from "office-casual";
...
```bash
office-casual embed report.pptx                          # → report.ocz.pptx
...
`office-casual locate report.ocz.xlsx <data-id>`（API `locate(node)` / `deepLink(node,file)`）で要素の位置を取得:
...
`StateGraph` で以下を配線（`src/agent/graph.ts`）。**3 階層**（純関数 / ローカル小型モデル / クラウド Claude）を混在させ、`MemorySaver` checkpointer で再開可能にする。

```
START → ingest → structural → embed → semantic → causal → verify ─┬→ END
       └─ 純関数 ─┘      (transformers.js)  └──── Claude ────┘   │
                                                  ↑              │
                                                  └─ (深掘りループ) ┘
```
...
- **semantic / causal / verify**: `ChatAnthropic`（既定 `claude-sonnet-4-6`、深掘りは `claude-opus-4-8`）。
- **causal は 2 モード**（§5.6）。

### 5.6 ハイブリッド因果: ローカルで絞り、Claude は裁定だけ

`causes` の候補空間は本来 O(n²)。これを全て Claude に投げるとコストが爆発する。そこで:

1. **embed（ローカル小型）** が埋め込み類似で高親和ペアだけに絞り、対称ペアを**無向に dedup**（実測 8→4）。
2. **causal（Claude）** は候補ペアごとに **「向き(a→b / b→a / none)・極性・メカニズム・evidence」だけを裁定**する。自由生成はしない → トークン/呼び出しを大幅削減。
3. **verify（Claude）** が残りを独立 N 票で敵対的に反証し confidence を確定。

埋め込み無効時は causal が**生成モード**（Claude が自由仮説）にフォールバックする。役割分担は「ローカル=広域スクリーニング / Claude=精密裁定」。
...
- **agent への配線**: `llm.provider:"webgpu-gemma"` で causal 段の裁定をローカル Gemma 4 が担当。
  `llm.verify:false` で Claude を一切使わない**完全ローカル**実行も可（verify=既定 true なら Claude が最終敵対検証）。
- **web デモ** (`web/index.html`): `.xlsx/.pptx/.docx` のドロップ or テキスト → OOXML 解析 → ①埋め込み選別 →
  ...
  WebGPU 非対応時は警告バナーを出し、**wasm フォールバック**（E2B 固定・候補上限8・max_new_tokens=48）で最低限動作。office-casual の dist を import map 経由でそのまま再利用。
...
```ts
// 完全ローカル (API キー不要): 埋め込み選別 + Gemma4 裁定, Claude verify なし
await analyze("Q1.pptx", {
  depth: "causal",
  llm: { provider: "webgpu-gemma", device: "webgpu", verify: false },
});
```

```ts
import { WebGpuGemmaAdjudicator } from "office-casual";
...
> モデル ID は環境準拠（Opus 4.8 / Sonnet 4.6 / Haiku 4.5）。最新かつ最適な Claude を既定とする。
...
3. **ローカル候補生成** — 埋め込み類似で `mentions`/`causes` 候補を閾値抽出（Claude を呼ぶ前の安価な一次選別）。

**役割分担**: 埋め込みは**無向の意味的親和度**を出す。**因果の向き・極性・採否は Claude verify** が裁定する（埋め込みだけでは A→B か B→A か決まらないため）。
...
```ts
import { analyze } from "office-casual";
...
```bash
npx office-casual analyze Q1-report.pptx --depth causal --out q1.casual.json
npx office-casual graph q1.casual.json --format dot | dot -Tsvg > q1.svg
```
...
- [ADR-0003](docs/adr/0003-edge-llm-tiering.md) — edge-LLM の 3 階層化（埋め込み → ローカル Gemma 4/WebGPU → Claude verify）と実測
...
6. **3 階層 edge-LLM** — ローカルで絞り・裁定し、Claude は最終 verify。
