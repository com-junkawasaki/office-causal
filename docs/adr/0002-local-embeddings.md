...
ADR-0001 で因果エッジの裁定を生成 LLM (verify) に委ねた。しかし全ノード対・全エッジに
生成 LLM を呼ぶのはコスト・レイテンシ的に非現実的。一方、構造/参照/依存エッジには
「意味的にどれだけ強いか」の指標が無く、causal 候補生成も生成 LLM 任せだった。

要件:
1. 各エッジに**意味的強度 (weight)** を安価に付けたい（"edge の小さい LLM weight"）。
2. data-id 単位で**タグ**を付け、分類・フィルタ・可視化に使いたい。
3. Gemma 4 裁定の前に**候補エッジをローカルで一次選別**したい。
...
API キー不要・ローカル完結。これを「小さい LLM」と位置づけ、ローカル Gemma 4 と階層化する。
...
### D3. 埋め込みは「無向の意味的親和度」、向き・極性・採否は Gemma 4

埋め込み類似は対称（A↔B 同値）なので、`proposeEdges` は候補と weight のみを出す。
**因果の向き (cause→effect)・極性 (+/-)・最終採否はローカル Gemma 4**（causal 裁定 + verify）が担う (ADR-0001 D5)。
役割分担: 埋め込み = 安価な広域スクリーニング / Gemma 4 = 精密な裁定・検証。すべてローカル。
...
- ✅ Gemma 4 裁定の前に候補を絞れる → 生成呼び出し削減。
- ✅ 全エッジに意味的 weight、全ノードに data-id タグ。多言語対応。
- ✅ オフライン安全（フォールバック）。クラウド非依存・API キー不要。
- ⚠️ 対称類似ゆえ候補は双方向に出る → 向きは Gemma 4 が決める前提（D3）。
- ⚠️ 量子化小型モデルの精度限界。重要判断は E4B + `verifyVotes` 増の Gemma 4 verify で補う。
