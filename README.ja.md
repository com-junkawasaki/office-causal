...
- [ADR-0005](docs/adr/0005-tensor-network.md) — コーパスを 4 階層 tensor network 化（Document → Page → Object → Causal、bond 次元 χ、文書横断 causes）
- [ADR-0006](docs/adr/0006-browser-demo-stack.md) — ブラウザデモを Svelte + Vite + GitHub Pages で、WebGPU→WASM 透過フォールバック、transformers のみ importmap external
